...
- **Workflow deep-read orchestration (new in v0.5.0)**: `prepare-workflow.mjs` + `tools/notes-workflow.js` (DSH workflow script) — the **parallel deep-read layer** for batch skeletons: one subagent per paper, strong schema validation + `check-note.mjs` content-level self-check (placeholders / `$` pairing / figure explanations / status), auto-retry on failure; wikilink consistency guaranteed by a pre-scanned Vault title list. Benchmarked: 8 papers, 8/8 passed
...
- **Figures with sections (new in v0.6.0)**: figures/tables are auto-mapped to their owning section by PDF page, with explanations **embedded right under the corresponding §** (plus a "figures involved" index line) — read text and figures side by side; unmapped figures collect in a trailing fallback area for manual placement
