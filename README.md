...
- **workflow 精读编排（v0.5.0 新！）**：`prepare-workflow.mjs` + `tools/notes-workflow.js`（DSH workflow 脚本）——批量骨架的**并行精读层**，每篇一个 subagent，schema 强校验 + `check-note.mjs` 内容级自检（占位符/$ 配对/图解说/status），失败自动重试，wikilink 由预扫描的 Vault 标题清单保证篇间一致。实测 8 篇一次 8/8 通过
- **图文并茂**：自动定位并裁剪每张图/表（题注感知），位图走嵌入图、矢量图走绘图包围盒，含 300dpi 高清导出与 `crop` 手工补图子命令
- **逐节精读模板**：每个 § 预留「主要内容 / 科学问题 / 方法 / 特征性质结论 / 研究价值 / **关键公式**」；每张图预留「图表内容 / 图内解说 / 科学内涵」三段解说，原题注自动引用
- **图文随章节（v0.6.0 新）**：图/表按 PDF 页码自动映射到所属章节，**解说直接嵌在对应 § 下**（含"涉及图表"索引行），阅读时图文对照无需翻页；未识别归属的图集中文末兜底区，可手动归位
- **自然语言与学术规范（v0.3.0 新）**：撰写时强制成段叙述、学术用词、术语一致、逻辑衔接、忠实原文（SKILL.md §7.1），杜绝占位注释与口语化表述
- **LaTeX 公式（v0.3.0 新）**：关键公式以行内 `$…$` / 行间 `$$…$$` 写入，交付前按 §7.3 自查清单校验渲染（美元配对、单位转义、希腊字母命令）；`vault.mjs` 自动提示 `$` 配对异常
